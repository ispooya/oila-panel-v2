...
@section('title', 'موارد استفاده محصول')
                    ...
                    <div class="card-body">
                        <div class="card-title border-bottom" style="margin-bottom: 1.5rem;">
                            <h6 class="card-title mb-3">
                                ...
                                افزودن موارد استفاده محصول
                            </h6>
                        </div>
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-6">
                                <form action="{{route('product.uses.store')}}" method="POST">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered text-center">
                                ...
                                @foreach($uses as $use)
                                    <tr class="us-row-{{$use->id}}">
                                        <td class="type"><img src="{{$use->icon }}" alt="" width="90"></td>
                                        <td class="type">{{$use->name}}</td>
                                        <td class="text-center">
                                            <span class="mr-1 "
                                                  data-toggle="tooltip"
                                                  title="حذف خبر">
                                                <a class="update-user"
                                                   href="/product/uses/destroy/{{$use->id}}"
    ...
    <script>
        $(document)
            .on('click', 'form button[type=submit]', function(e) {
                if(!$('#img').val()) {
                    window.alert("لطفا تصویر را وارد کنید.")
            ...
            $("#dZUpload").dropzone({
                ...
                init: function () {
                    ...
                    this.on('sending', function (file, xhr, formData) {
                        formData.append('type', 'product-uses');
